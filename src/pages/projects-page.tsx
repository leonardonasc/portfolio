                            ...
                            <div className="grid gap-5 md:grid-cols-2">
                                {projects.filter((project) => project.category === category).map((project) => (
                                    <Link key={project.slug} to={`/projects/${project.slug}`} className="group border border-border bg-foreground">
                                        <div className="aspect-16/8 overflow-hidden bg-[#24292d]"><img src={project.image} srcSet={project.imageSrcSet} sizes="(min-width: 768px) 50vw, 100vw" alt={`Imagem do projeto ${project.title}`} loading="lazy" decoding="async" width="1280" height="640" className="block h-full w-full bg-[#24292d] object-cover transition duration-700 group-hover:scale-105" /></div>
